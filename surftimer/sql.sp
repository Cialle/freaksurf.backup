		...
		while (SQL_FetchRow(hndl))
		{
			...
			for (int i = 0; i < GetArraySize(g_MapList); i++) // Check that the map is in the mapcycle
			{
				...
				if (StrEqual(szMapName2, szMap, false))
				{
					...
					{
						wrpoints = ((float(totalplayers) * 100));
						wrpoints += 3500;
					}
					else if (tier == 7)
					{
						wrpoints = ((float(totalplayers) * 6666));
						wrpoints += 6666;
					}
					else if (tier == 8)
					{
						wrpoints = ((float(totalplayers) * 666666));
						wrpoints += 666666;
					}
					else // no tier set
						wrpoints = 25.0;
...
public void db_insertZone(int zoneid, int zonetype, int zonetypeid, float pointax, float pointay, float pointaz, float pointbx, float pointby, float pointbz, int vis, int team, int zonegroup)
{
	...
	Format(szQuery, 1024, sql_insertZones, g_szMapName, zoneid, zonetype, zonetypeid, pointax, pointay, pointaz, pointbx, pointby, pointbz, vis, team, zonegroup, zName, "None", "player", 0, 350.0);
